...
NeuralNetwork.prototype.initNeurons = function (inputVertices) {
  ...
  for (
    var i = 0;
    i < inputVertices.length;
    i += this.settings.verticesSkipStep
  ) {
  ...
  for (var i = 0; i < this.staticSignals.length; i++) {
    ...
    var n = new Neuron(
      ...
      pos.y,
      pos.z,
      this.staticSignals[i].axon_effect
    );
    this.components.neurons.push(n);
    this.neuronsGeom.vertices.push(n);
    ...
    var signalData = {
      idx: i,
      neuron_id: neuronId,
      visible: this.staticSignals[i].signalVisible,
      interval: this.staticSignals[i].interval / 1000,
      color: this.staticSignals[i].pColor,
      size: this.staticSignals[i].signalSize,
    };
  ...
  for (var ii = 0; ii < this.activeSignalData.length; ii++) {
    var pp = new ParticlePool(
      this.settings.limitSignals,
      this.activeSignalData[ii].visible,
      this.activeSignalData[ii].size,
      this.activeSignalData[ii].color,
      this.activeSignalData[ii].interval
    );
    this.meshComponents.add(pp.meshComponents);
    this.particlePools.push(pp);

    //Set color in active neuron
    this.components.neurons[this.activeSignalData[ii].neuron_id].color =
      this.activeSignalData[ii].color;
  }
  ...
  // neuron mesh
  this.neuronParticles = new THREE.PointCloud(
    this.neuronsGeom,
    this.neuronShaderMaterial
  );
...
NeuralNetwork.prototype.initAxons = function () {
  ...
  for (var j = 0; j < allNeuronsLength; j++) {
    ...
    for (var k = j + 1; k < allNeuronsLength; k++) {
      ...
      if (
        n1 !== n2 &&
        n1.distanceTo(n2) < this.settings.maxAxonDist &&
  ...
  this.axonGeom.addAttribute(
    "index",
    new THREE.BufferAttribute(axonIndices, 1)
  );
  this.axonGeom.addAttribute(
    "position",
    new THREE.BufferAttribute(axonPositions, 3)
  );
  this.axonGeom.addAttribute(
    "opacity",
    new THREE.BufferAttribute(axonOpacities, 1)
  );
  ...
  this.axonMesh = new THREE.Line(
    this.axonGeom,
    this.axonShaderMaterial,
    THREE.LinePieces
  );
...
NeuralNetwork.prototype.initConboxes = function () {
  for (var i = 0; i < DATASET.length; i++) {
    ...
    var boxPos = new THREE.Vector3(
      DATASET[i].x * contextBoxSize - brainSizeX / 2 + contextBoxSize / 2,
      DATASET[i].y * contextBoxSize - brainSizeY / 2 + contextBoxSize / 2,
      DATASET[i].z * contextBoxSize - brainSizeZ / 2 + contextBoxSize / 2
    );
    ...
    for (var j = 0; j < DATASET[i].signals.length; j++) {
      ...
      var spos = new THREE.Vector3(
        boxPos.x -
          contextBoxSize / 2 +
          (signalInfo.position.x * contextBoxSize) / 100,
        boxPos.y -
          contextBoxSize / 2 +
          (signalInfo.position.y * contextBoxSize) / 100,
        boxPos.z -
          contextBoxSize / 2 +
          (signalInfo.position.z * contextBoxSize) / 100
      );
      var s = new StaticSignal(
        signalInfo.visible,
        spos,
        signalInfo.size,
        signalInfo.color,
        signalInfo.interval,
        signalInfo.axon_effect
      );
...
NeuralNetwork.prototype.update = function (deltaTime) {
  ...
  for (var ii = 0; ii < this.activeSignalData.length; ii++) {
    this.components.neurons[this.activeSignalData[ii].neuron_id].signalTimer +=
      deltaTime;
    if (
      this.components.neurons[this.activeSignalData[ii].neuron_id].signalTimer >
      this.activeSignalData[ii].interval
    ) {
      //this.components.neurons[this.activeSignalData[ii].neuron_id].activeSignalCount === 0 &&
      this.components.neurons[this.activeSignalData[ii].neuron_id].reset();
      this.releaseSignalAt(
        this.activeSignalData[ii].idx,
        this.components.neurons[this.activeSignalData[ii].neuron_id]
      );
    }
  }
...
NeuralNetwork.prototype.resetAllNeurons = function () {
  this.numPassive = 0;
  for (var ii = 0; ii < this.components.neurons.length; ii++) {
